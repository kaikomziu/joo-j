...
async function jooLoadAdminArticles() {
  const { data, error } = await jooSupabase.from('joo_articles').select('id, title, object_class, danger_level, ai_disclosure, rating_up, rating_down').order('id');
  ...
  tbody.innerHTML = data.map(a => {
    ...
    return `<tr>
      ...
      <td style="color:${aiInfo.color}">${aiInfo.label}</td>
      <td>${score}</td>
