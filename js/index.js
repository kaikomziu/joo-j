...
async function jooInitIndex() {
  ...
  const { data, error } = await jooSupabase
    .from('joo_articles')
    .select('id, number, title, object_class, danger_level, ai_disclosure, rating_up, rating_down, created_at')
  ...
  document.getElementById('joo-filter-ai').addEventListener('change', jooRenderGrid);
}
...
function jooRenderGrid() {
  ...
  const aiFilter = document.getElementById('joo-filter-ai').value;
  const filtering = !!(search || classFilter || dangerFilter || aiFilter);
  ...
  for (let n = 1; n <= slotCount; n++) {
    ...
    if (article) {
      ...
      if (aiFilter && article.ai_disclosure !== aiFilter) continue;
...
function jooSlotCard(n, article) {
  ...
  const aiInfo = jooAiInfo(article.ai_disclosure);
  ...
  div.innerHTML = `
    ...
    <div style="display:flex; gap:4px; flex-wrap:wrap;">
      ...
      <span class="joo-badge" style="color:${aiInfo.color}">${aiInfo.label}</span>
    </div>
  `;
